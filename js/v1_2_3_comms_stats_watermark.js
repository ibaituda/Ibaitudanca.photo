(function(){
  ...
  async function sendEmailRow(row){
    ...
    if(webhook){
      ...
      await client.from('email_outbox').update({status:'approved_pending_provider'}).eq('id',row.id);
      alert(t('Email aprobado. Falta configurar webhook/Resend para envío real.','Email approved. Configure webhook/Resend for real delivery.'));
    }
  }
  function rowHtml(r){
